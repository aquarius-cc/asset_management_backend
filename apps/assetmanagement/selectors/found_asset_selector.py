...
class FoundAssetSelector:
    ...
    @staticmethod
    def get_queryset_for_user(user: Any) -> QuerySet[FoundAsset]:
        """RBAC 行级过滤"""
        return get_asset_linked_queryset_for_user(user, FoundAsset.objects.for_list().filter(is_deleted=False))  # type: ignore[attr-defined,no-any-return]

    @staticmethod
    def get_found_assets_for_list() -> QuerySet[FoundAsset]:
        return FoundAsset.objects.for_list().filter(is_deleted=False)  # type: ignore[attr-defined,no-any-return]

    @staticmethod
    def get_found_assets_with_details() -> QuerySet[FoundAsset]:
        return FoundAsset.objects.with_asset_details().filter(is_deleted=False)  # type: ignore[attr-defined,no-any-return]

    @staticmethod
    def get_found_asset_by_recordcode(recordcode: str) -> FoundAsset | None:
        try:
            return FoundAsset.objects.with_asset_details().get(recordcode=recordcode, is_deleted=False)  # type: ignore[attr-defined,no-any-return]
    ...
    @staticmethod
    def get_by_asset_code(asset_code: str, user: Any = None) -> QuerySet[FoundAsset]:
