...
class TestDamagedAssetSelector:
    ...
    def test_get_asset_recordcode_for_update_hit(self, asset):
        """命中:单查询返回加锁实例,可在事务内使用(CT-1)"""
        from django.db import transaction

        damaged = DamagedAsset.objects.create(asset_recordcode=asset)
        with transaction.atomic():
            result = DamagedAssetSelector.get_asset_recordcode_for_update(asset.recordcode)
        assert result.recordcode == damaged.recordcode
        assert result.asset_recordcode_id == asset.recordcode

    def test_get_asset_recordcode_for_update_missing_raises(self):
        """缺失:抛 DoesNotExist(CT-1 语义锁)"""
        with pytest.raises(DamagedAsset.DoesNotExist):
            DamagedAssetSelector.get_asset_recordcode_for_update("NONEXIST")

    def test_get_asset_recordcode_for_update_soft_deleted_raises(self, asset):
        """软删除:等同不存在,抛 DoesNotExist(CT-1)"""
        damaged = DamagedAsset.objects.create(asset_recordcode=asset)
        damaged.delete()
        with pytest.raises(DamagedAsset.DoesNotExist):
            DamagedAssetSelector.get_asset_recordcode_for_update(asset.recordcode)
