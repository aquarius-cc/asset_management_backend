...
| 约定项      | 强制要求                                                               |
| -------- | ------------------------------------------------------------------ |
| **字符编码** | 所有字符串字段默认 `UTF-8`，支持 emoji                                          |
| **字段命名** | 下划线小写 `snake_case`（如 `asset_code`）；禁止中文或特殊字符                       |
| **主键**   | 统一 `id`，类型 `BIGSERIAL`（除非显式声明其他主键）                                |
| **时间字段** | `TIMESTAMP`（UTC+8），创建/更新时间使用 `auto_now_add` / `auto_now`           |
...
models.Model
  └── TimestampModel (abstract)
        └── BaseModel (abstract)
              ...
              ├── WasteAsset
              ├── HardDiskSN
              ├── BrokenAsset       (V4.0 新增：已损坏记录)
              ├── LostAsset         (V4.0 新增：已遗失记录)
              ├── FoundAsset        (V4.0 新增：资产找回记录)
              ├── UnregisteredAsset

models.Model（直接继承，不继承BaseModel）
  ├── Department
  └── Employee
...
**Choices**: `STORAGE_TYPE_CHOICES = [("newasset", "新货仓库"), ("recycle", "回收仓库"), ("broken", "损坏存放出库"), ("damaged", "待报废仓库")]`
...
**Choices**: `CONTRACT_SETTLEMENT_CHOICES = [("pending", "待结算"), ("settling_up", "结算中"), ("settled", "已结算")]`
...
| `asset_type_recordcode`       | **ForeignKey** | → AssetType, `to_field="recordcode"`, `on_delete=DO_NOTHING`, `related_name="assets"`                              | 关联资产类型        |
| `asset_contract_recordcode`   | **ForeignKey** | → Contract, `to_field="recordcode"`, `on_delete=SET_NULL`, `related_name="assets"`, `null=True, blank=True`           | 关联合同          |
...
| `asset_storage_recordcode`    | **ForeignKey** | → Storage, `to_field="recordcode"`, `on_delete=DO_NOTHING`, `related_name="assets"`, `blank=True, null=True`     | 所在仓库          |
| `asset_entry_person_recordcode` | **ForeignKey** | → Employee, `to_field="recordcode"`, `on_delete=SET_NULL`, `related_name="assets_entry"`, `blank=True, null=True`     | 入库人           |
| `asset_applicant_recordcode`  | **ForeignKey** | → Employee, `to_field="recordcode"`, `on_delete=DO_NOTHING`, `related_name="assets_applicant"`, `null=True, blank=True` | 申请人           |
| `asset_manager_recordcode`    | **ForeignKey** | → Employee, `to_field="recordcode"`, `on_delete=DO_NOTHING`, `related_name="assets_manager"`, `null=True, blank=True`   | 保管人           |
...
**Choices**: `ASSET_STATUS_CHOICES = [("in_store", "在库"), ("recycled_pending", "已回收待发放"), ("in_use", "在用"), ("broken", "已损坏"), ("damaged", "待报废"), ("scrapped", "已报废")]`

**外键关系（6个）**:

| 外键字段                       | 关联模型      | to_field   | related_name     | on_delete |
| ------------------------- | --------- | ---------- | ---------------- | --------- |
| `asset_type_recordcode`   | AssetType | recordcode | assets           | DO_NOTHING |
| `asset_contract_recordcode` | Contract  | recordcode | assets           | SET_NULL  |
| `asset_storage_recordcode`  | Storage   | recordcode | assets           | DO_NOTHING  |
| `asset_entry_person_recordcode` | Employee  | recordcode | assets_entry     | SET_NULL  |
| `asset_applicant_recordcode` | Employee  | recordcode | assets_applicant | DO_NOTHING  |
| `asset_manager_recordcode` | Employee  | recordcode | assets_manager   | DO_NOTHING  |
...
| `asset_recordcode`         | **ForeignKey** | → Asset, `to_field="recordcode"`, `related_name="out_assets"`, `on_delete=SET_NULL`, `null=True, blank=True`          | 关联资产     |
...
| `outasset_recordcode`       | **OneToOneField** | → OutAsset, `to_field="recordcode"`, `on_delete=CASCADE`, `related_name="recycle_record"`                                    | 关联出库记录   |
| `asset_recordcode`          | **ForeignKey**    | → Asset, `to_field="recordcode"`, `related_name="recycle_assets"`, `on_delete=PROTECT`                                       | 回收的资产    |
...
| `is_broken`                 | BooleanField      | `default=False`                                                                                                              | 是否损坏回收（V4.0 新增） |
| `is_lost`                   | BooleanField      | `default=False`                                                                                                              | 是否遗失回收（V4.0 新增） |
| `version`                   | IntegerField      | `default=1`                                                                                                                  | 乐观锁版本号   |
...
| --------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `recordcode`                | CharField         | `max_length=32, unique=True, blank=True, null=True`                                                                          | 后端生成的全局唯一编码 |
| `asset_recordcode`          | **OneToOneField** | → Asset, `to_field="recordcode"`, `related_name="damaged_asset"`, `on_delete=SET_NULL`, `null=True, blank=True`              | 待报废的资产      |
...
| `original_status`           | CharField         | `max_length=20, blank=True, null=True`                                                                                       | 进入damaged前的资产状态，用于reject时回退（V4.0 新增） |
| `version`                   | IntegerField      | `default=1`                                                                                                                  | 乐观锁版本号      |
...
| ------------------------- | ----------------- | -------------------------------------------------------------------------------------------------------------------------- | ----------- |
| `recordcode`              | CharField         | `max_length=32, unique=True, blank=True, null=True`                                                                        | 后端生成的全局唯一编码 |
| `asset_recordcode`        | **OneToOneField** | → Asset, `to_field="recordcode"`, `related_name="waste_asset"`, `on_delete=DO_NOTHING`                                     | 已报废的资产      |
| `damaged_recordcode`      | **OneToOneField** | → DamagedAsset, `to_field="recordcode"`, `related_name="waste_asset_record"`, `on_delete=DO_NOTHING`, `null=True, blank=True` | 来源待报废记录    |
...
### 10. BrokenAsset（已损坏资产记录）

**db_table**: `am_broken_asset` | **继承**: `BaseModel` | **RECORDCODE_PREFIX**: `BROKEN`

| 字段名 | 字段类型 | 参数 | 说明 |
|-------|---------|------|------|
| `asset_recordcode` | **OneToOneField** | → Asset, `to_field="recordcode"`, `on_delete=SET_NULL`, `null=True, blank=True` | 损坏资产编码 |
| `broken_date` | DateField | `default=timezone.now` | 损坏日期 |
| `operator_employee` | **ForeignKey** | → Employee, `to_field="recordcode"`, `on_delete=SET_NULL`, `null=True, blank=True` | 操作人 |
| `broken_reason` | CharField | `max_length=100` | 损坏原因 |
| `broken_description` | TextField | `blank=True, null=True` | 损坏描述 |
| `version` | IntegerField | `default=1` | 乐观锁版本号 |

---

### 11. LostAsset（已遗失资产记录）

**db_table**: `am_lost_asset` | **继承**: `BaseModel` | **RECORDCODE_PREFIX**: `LOST`

| 字段名 | 字段类型 | 参数 | 说明 |
|-------|---------|------|------|
| `asset_recordcode` | **OneToOneField** | → Asset, `to_field="recordcode"`, `on_delete=SET_NULL`, `null=True, blank=True` | 遗失资产编码 |
| `lost_date` | DateField | `default=timezone.now` | 遗失日期 |
| `operator_employee` | **ForeignKey** | → Employee, `to_field="recordcode"`, `on_delete=SET_NULL`, `null=True, blank=True` | 操作人 |
| `last_known_location` | CharField | `max_length=200, blank=True, null=True` | 最后已知位置 |
| `lost_reason` | CharField | `max_length=100` | 遗失原因 |
| `lost_description` | TextField | `blank=True, null=True` | 遗失描述 |
| `version` | IntegerField | `default=1` | 乐观锁版本号 |

---

### 12. FoundAsset（资产找回记录）

**db_table**: `am_found_asset` | **继承**: `BaseModel` | **RECORDCODE_PREFIX**: `FOUND`

| 字段名 | 字段类型 | 参数 | 说明 |
|-------|---------|------|------|
| `lost_asset_recordcode` | **OneToOneField** | → LostAsset, `to_field="recordcode"`, `on_delete=CASCADE` | 关联遗失记录 |
| `asset_recordcode` | **ForeignKey** | → Asset, `to_field="recordcode"`, `on_delete=PROTECT` | 找回资产编码 |
| `found_date` | DateField | `default=timezone.now` | 找回日期 |
| `found_location` | CharField | `max_length=200, blank=True, null=True` | 找回地点 |
| `operator_employee` | **ForeignKey** | → Employee, `to_field="recordcode"`, `on_delete=SET_NULL`, `null=True, blank=True` | 操作人 |
| `found_description` | TextField | `blank=True, null=True` | 找回描述 |
| `version` | IntegerField | `default=1` | 乐观锁版本号 |

---

### 13. AssetOperationLog（资产操作记录 — 只读表）
...
| `asset_name`          | CharField             | `max_length=100, null=True, blank=True`                                         | 资产名称（冗余存储，删除 Asset 不影响记录）      |
| `asset_specification` | CharField             | `max_length=100, null=True, blank=True`                                         | 资产规格（冗余存储，删除 Asset 不影响记录）      |
...
**Choices**: `OPERATION_TYPE_CHOICES = [("create","创建"),("update","更新"),("delete","删除"),("out","出库"),("recycle","回收"),("broken","已损坏"),("lost","已遗失"),("found","找回"),("damaged","待报废"),("waste","已报废"),("approve","审批"),("transfer","转移"),("state_change","状态变更")]`

**特殊约束**: `save()` 中若已存在记录则抛 `PermissionError` 阻止更新；`delete()` 直接抛 `PermissionError` 阻止删除。

**设计说明**: `asset_name` 和 `asset_specification` 为冗余存储字段，删除 Asset 记录不会影响操作日志中的资产名称和规格信息。
...
| `app_label`      | CharField             | `max_length=50, db_index=True`                  | 操作所属应用（department/employee/authuser） |
| `operation_type` | CharField             | `max_length=20, choices=OPERATION_TYPE_CHOICES, db_index=True` | 操作类型          |
| `logging_id`     | CharField             | `max_length=50, unique=True, db_index=True, blank=True` | 日志唯一标识        |
| `operation_time` | DateTimeField         | `auto_now_add=True, db_index=True`              | 操作时间            |
| `operator_jobcode` | CharField           | `max_length=20, blank=True, null=True`          | 操作人工号           |
...
| `parent`                 | ForeignKey   | `to='self', to_field='recordcode', null=True`       | 上级部门 FK（指向 recordcode） |
| `path`                   | CharField    | `max_length=500, default=''`                        | 物化路径，如 /WHRT/JTZNBM/JTBGS |
| `parent_code`            | CharField    | `max_length=20, null=True, blank=True`              | 【已废弃】上级部门编码（旧） |
...
| ----------------- | ------------------------------ | ------------ | ---------- | ------------------- |
| Asset             | asset_type_recordcode          | AssetType    | recordcode | DO_NOTHING          |
| Asset             | asset_contract_recordcode      | Contract     | recordcode | SET_NULL            |
| Asset             | asset_storage_recordcode       | Storage      | recordcode | DO_NOTHING          |
| Asset             | asset_entry_person_recordcode  | Employee     | recordcode | SET_NULL            |
| Asset             | asset_applicant_recordcode     | Employee     | recordcode | DO_NOTHING          |
| Asset             | asset_manager_recordcode       | Employee     | recordcode | DO_NOTHING          |
| OutAsset          | asset_recordcode               | Asset        | recordcode | SET_NULL            |
| RecycleAsset      | outasset_recordcode            | OutAsset     | recordcode | CASCADE (OneToOne)  |
| RecycleAsset      | asset_recordcode               | Asset        | recordcode | PROTECT             |
| RecycleAsset      | operator_employee              | Employee     | recordcode | SET_NULL            |
| DamagedAsset      | asset_recordcode               | Asset        | recordcode | SET_NULL (OneToOne) |
| DamagedAsset      | approver                       | Employee     | recordcode | SET_NULL            |
| WasteAsset        | asset_recordcode               | Asset        | recordcode | DO_NOTHING (OneToOne) |
| WasteAsset        | damaged_recordcode             | DamagedAsset | recordcode | DO_NOTHING (OneToOne) |
...
| UnregisteredAsset | result_recycle_asset           | RecycleAsset | recordcode | SET_NULL            |
| UnregisteredAsset | result_damaged_asset           | DamagedAsset | recordcode | SET_NULL            |
...
- **字段**: asset_code, asset_name, asset_brand, asset_unit, asset_specification, asset_purchase_price, asset_purchase_date, asset_warranty_period, asset_current_status, asset_description, asset_using_location, asset_entry_date, asset_type_recordcode(FK), asset_contract_recordcode(FK), asset_storage_recordcode(FK), asset_entry_person_recordcode(FK), asset_applicant_recordcode(FK), asset_manager_recordcode(FK), is_active
...
- **嵌套序列化器**: asset_type_recordcode→AssetTypeSerializer, asset_contract_recordcode→ContractSerializer, asset_storage_recordcode→StorageSerializer, asset_entry_person_recordcode→EmployeeSerializer, asset_applicant_recordcode→EmployeeSerializer, asset_manager_recordcode→EmployeeSerializer, harddisk_sns→HardDiskSNSimpleSerializer(many)

#### AssetCreateSerializer

- **模型**: Asset | **继承**: ModelSerializer
- **排除字段**: ["recordcode"]
- **SlugRelatedField**: asset_type_recordcode(slug_field="asset_type_code"), asset_contract_recordcode(slug_field="contract_code"), asset_storage_recordcode(slug_field="storage_code"), asset_entry_person_recordcode(slug_field="employee_jobcode"), asset_applicant_recordcode(slug_field="employee_jobcode"), asset_manager_recordcode(slug_field="employee_jobcode")
...
- **字段**: id, recordcode, recycle_asset_date, recycle_asset(FK), recycle_outasset(SlugRelated, slug_field="recordcode"), recycle_asset_number, recycle_type, is_active
...
- **write_only字段**: recycle_asset_recycle_person_jobcode(SlugRelated, slug_field="employee_jobcode"), recycle_asset_storage(SlugRelated, slug_field="storage_code")
...
- **所有字段**: read_only
- **额外声明字段**: operation_type_display, asset_name(source='operation_asset.asset_name'), asset_specification(source='operation_asset.asset_specification')
...
- **RecycleAssetBatchItemSerializer 字段**:
  - row_number: IntegerField (optional) — 行号，用于错误定位
  - recycle_outasset_code: SlugRelatedField(slug_field="recordcode") — 出库记录编码（必填）
  - recycle_date: DateField (optional) — 回收日期
  - recycle_type: CharField — 回收原因（必填）
  - recycle_description: CharField (optional) — 回收描述
...
| HTTP方法 | URL                                                         | 功能     | 请求参数                                                                                                                          | 返回序列化器                             | 权限              |
| ------ | ----------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | --------------- |
| GET    | `/api/assets/recycle-assets/`                               | 列表     | query: page, page_size, recycle_asset, operator_employee, recycle_outasset, recycle_date_from, recycle_date_to, ordering | RecycleAssetSerializer             | IsAuthenticated |
