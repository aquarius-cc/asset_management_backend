...
class DjangoEmployeeProvider(EmployeeProvider):
    ...
    def get_all_employees(self):
        return list(
            Employee.objects.filter(is_deleted=False).values("employee_jobcode", "employee_name", "employee_department")
        )
...
class AssetStateMachine:
    ...
    def can_transition(self, current_state: AssetState, trigger: str, context: Optional[Dict] = None) -> bool:
        """检查是否可以执行状态转换"""
    ...
    def transition(self, asset: "Asset", trigger: str, context: Optional[Dict] = None) -> "StateTransitionResult":
        """执行状态转换"""
        ...
        if not transitions:
            raise InvalidTransitionError(f"无法从 {current_state.value} 通过 {trigger} 转换状态")
        ...
        for transition in transitions:
            ...
            return StateTransitionResult(
                success=True, from_state=old_state, to_state=transition.to_state.value, trigger=trigger
            )
...
class AssetService:
    ...
    def change_asset_status(asset_code: str, new_status: str, trigger: str, operator=None):
        ...
        context = {"asset": asset, "operator": operator, "new_status": new_status}

        # 使用状态机执行转换
        result = AssetService._state_machine.transition(asset=asset, trigger=trigger, context=context)
...
class AssetStatePublisher:
    ...
    def publish_state_change(event: AssetStateChangeEvent):
        """发布状态变更事件"""
        asset_state_changed.send(sender=AssetStatePublisher, event=event)
```
...
class AuditLogger:
    ...
    @staticmethod
    def log_asset_create(asset, operator_jobcode: Optional[str] = None, operator_name: Optional[str] = None) -> bool:
        """
        记录资产创建日志
    ...
    def log_asset_delete(
        asset_code: str, asset_name: str, operator_jobcode: Optional[str] = None, operator_name: Optional[str] = None
    ) -> bool:
        """记录资产删除日志"""
        return AuditLogger._safe_log(
...
class AssetService:
    ...
    @transaction.atomic
    def create_asset(
        asset_data: Dict[str, Any], operator_jobcode: Optional[str] = None, operator_name: Optional[str] = None
    ) -> Asset:
        """
        创建资产
        ...
        # ✅ 显式记录日志 - 调试时可在此处打断点
        AuditLogger.log_asset_create(asset=asset, operator_jobcode=operator_jobcode, operator_name=operator_name)
    ...
    @transaction.atomic
    def delete_asset(
        asset_code: str, operator_jobcode: Optional[str] = None, operator_name: Optional[str] = None
    ) -> None:
        """
        删除资产（软删除）
...
class OutAssetService:
    ...
    def create_outasset(outasset_data: Dict[str, Any]) -> OutAsset:
        ...
        if asset.asset_current_status not in ["in_store", "recycled_pending"]:
            raise AppValidationError(detail=f"资产当前状态为 {asset.asset_current_status}，不能出库")
...
class AssetService:
    ...
    def update_asset(
        ...
        # 验证更新数据
        valid_data, invalid_data = FieldPermissionManager.validate_update_data(Asset, update_data)

        if invalid_data:
            raise AppValidationError(detail=f"字段验证失败: {', '.join(invalid_data.values())}")
...
# 生成Asset白名单
asset_whitelist = generate_whitelist(Asset, exclude_fields=["asset_code", "asset_qrcode", "asset_current_status"])
