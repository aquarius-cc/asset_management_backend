...
class TestApproveAssetRecordcode:
    ...
    def test_approve_nonexistent_raises(self, user):
        with pytest.raises(AppValidationError) as exc_info:
            DamagedAssetService.approve_asset_recordcode(
                "NONEXIST", user.employee_jobcode, "审批人"
            )
    ...
    def test_approve_non_pending_status_raises(self, asset_damaged, user):
        ...
        with pytest.raises(AppValidationError) as exc_info:
            DamagedAssetService.approve_asset_recordcode(
                asset_damaged.recordcode, user.employee_jobcode, "审批人"
            )
...
class TestRejectAssetRecordcode:
    def test_reject_to_broken(self, db, storage, asset_type, user):
        ...
        asset = Asset.objects.create(
            asset_code="A_BRK_R", asset_name="损坏资产",
            asset_purchase_price=2000, asset_purchase_date="2024-01-01",
            asset_entry_date="2024-01-01",
            asset_storage_recordcode=storage, asset_type_recordcode=asset_type,
            asset_current_status="damaged",
        )
        damaged = DamagedAsset.objects.create(
            asset_recordcode=asset, damaged_asset_number=1,
            approval_status="pending", original_status="broken",
        )
    ...
    def test_reject_to_lost(self, db, storage, asset_type, user):
        ...
        asset = Asset.objects.create(
            asset_code="A_LOST_R", asset_name="遗失资产",
            asset_purchase_price=2000, asset_purchase_date="2024-01-01",
            asset_entry_date="2024-01-01",
            asset_storage_recordcode=storage, asset_type_recordcode=asset_type,
            asset_current_status="damaged",
        ...
        damaged = DamagedAsset.objects.create(
            asset_recordcode=asset, damaged_asset_number=1,
            approval_status="pending", original_status="lost",
        )
        result = DamagedAssetService.reject_asset_recordcode(
            asset.recordcode,
            approver_jobcode=user.employee_jobcode,
