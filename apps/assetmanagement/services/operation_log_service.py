...
from apps.assetmanagement.models import AssetOperationLog, Asset
...
class OperationLogService:
    ...
    def log_operation(
        ...
        【新增】自动关联 Asset 对象，支持 select_related 优化查询
        ...
        asset_instance = None
        try:
            asset_instance = Asset.objects.get(asset_code=asset_code)
        except Asset.DoesNotExist:
            logger.warning(f"操作日志关联资产不存在: {asset_code}")
    ...
    def log_asset_create(
        ...
        after_data = {
            ...
            "asset_storage": str(asset.asset_storage) if asset.asset_storage else None,
        }
    ...
    def log_asset_recycle(
        ...
        Args:
            asset: 资产对象
            recordcode: 回收记录编码（BaseModel.recordcode）
            operator_jobcode: 操作人工号
            operator_name: 操作人姓名
