...
class TestDamagedCreateRowIsolation:
    ...
    def test_cross_dept_create_rejected(
        self, api_client: APIClient, dept_b_asset_recyclable: Any, regular_a_user: Any
    ) -> None:
        """甲部门 regular 用户对乙部门资产提交报废申请 → 拒绝(修复前 201)"""
...
class TestLifecycleBatchCreateRowIsolation:
    ...
    def test_broken_cross_dept_rejected(self, api_client: APIClient, dept_b_asset: Any, regular_a_user: Any) -> None:
        """甲部门用户对乙部门资产批量标记损坏 → 记入 fail_items(隔离校验拒绝)"""
        api_client.force_authenticate(user=regular_a_user)
    ...
    def test_broken_own_dept_allowed(self, api_client: APIClient, owned_asset: Any, regular_a_user: Any) -> None:
        """本部门资产批量标记损坏(防误伤)"""
        api_client.force_authenticate(user=regular_a_user)
        url = reverse("broken-assets-batch-create")
        response = api_client.post(
    ...
    def test_lost_cross_dept_rejected(self, api_client: APIClient, dept_b_asset: Any, regular_a_user: Any) -> None:
        """甲部门用户对乙部门资产批量标记遗失 → 记入 fail_items(隔离校验拒绝)"""
        api_client.force_authenticate(user=regular_a_user)
    ...
    def test_lost_own_dept_allowed(self, api_client: APIClient, owned_asset: Any, regular_a_user: Any) -> None:
        """本部门资产批量标记遗失(防误伤)"""
        api_client.force_authenticate(user=regular_a_user)
        url = reverse("lost-assets-batch-create")
        response = api_client.post(
