...
from .models import AssetOperationLog
from .operation_log_service import OperationLogQueryService
from .serializers import AssetOperationLogSerializer
...
class AssetHistoryView(ResponseWrapperMixin, APIView):
    ...
    def get(self, request, asset_code: str) -> Response:
        ...
        if not logs:
            return error_response(
                message=f"资产 {asset_code} 没有操作记录",
                status_code=status.HTTP_404_NOT_FOUND
            )
...
class AssetStatusTimelineView(ResponseWrapperMixin, APIView):
    ...
    def get(self, request, asset_code: str) -> Response:
        ...
        if not timeline:
            return error_response(
                message=f"资产 {asset_code} 没有状态变更记录",
                status_code=status.HTTP_404_NOT_FOUND
            )
...
class UserOperationsView(ResponseWrapperMixin, APIView):
    ...
    def get(self, request, operator_jobcode: str) -> Response:
        ...
        if not logs:
            return error_response(
                message=f"用户 {operator_jobcode} 没有操作记录",
                status_code=status.HTTP_404_NOT_FOUND
            )
